...
out_dir <- "/Users/bamflappy/PfrenderLab/RNA_evolution/outputs/tables_and_figures/family1_co_occuring_bases/alternates_b37"
...
###
## most common base (base 37 = A) alternate U
###

# subset to counts data
cluster_data_conserved <- cluster_data[substr(cluster_data$sequence, 37, 37) == "T", ]
...
# save the plot
exportFile <-"family_1_base_conservation_b37_U.png"
...
write.csv(cluster_data_conserved_out, "family_1_sequences_b37_U.csv", quote = FALSE, row.names = FALSE)

###
## most common base (base 37 = A) alternate G
###

# subset to counts data
cluster_data_conserved <- cluster_data[substr(cluster_data$sequence, 37, 37) == "G", ]
...
base_counts_plot <- ggplot(data = base_counts, aes(reorder(as.character(base_ID), base_ID), base, fill= conservation_na)) + 
  ...
  geom_text(aes(label = round(conservation_na, digits = 2)), size = 4, colour =ifelse(base_counts$conservation_na < 50, "black", "white"))
# save the plot
exportFile <-"family_1_base_conservation_b37_C.png"
png(exportFile, units="in", width=20, height=3, res=300)
print(base_counts_plot)
dev.off()
...
# write sequences to file
write.csv(cluster_data_conserved_out, "family_1_sequences_b37_C.csv", quote = FALSE, row.names = FALSE)

###
## most common base (base 37 = A) alternate C
###

# subset to counts data
cluster_data_conserved <- cluster_data[substr(cluster_data$sequence, 37, 37) == "C", ]
...
base_counts_plot <- ggplot(data = base_counts, aes(reorder(as.character(base_ID), base_ID), base, fill= conservation_na)) + 
  ...
  geom_text(aes(label = round(conservation_na, digits = 2)), size = 4, colour =ifelse(base_counts$conservation_na < 50, "black", "white"))
# save the plot
exportFile <-"family_1_base_conservation_b37_A.png"
png(exportFile, units="in", width=20, height=3, res=300)
print(base_counts_plot)
dev.off()
...
# write sequences to file
write.csv(cluster_data_conserved_out, "family_1_sequences_b37_A.csv", quote = FALSE, row.names = FALSE)
